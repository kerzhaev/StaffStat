...
- **Центральный модуль:** `mod_Maintenance_Logic` — единая точка входа для всего I/O настроек и проверок здоровья данных; надёжная работа с DAO, идемпотентное создание схемы при повторном запуске.
